...
def bootstrap_kurtosis_test(vi_matrix, interaction_loci_indices, non_interaction_loci_indices, n_bootstrap=5000):
    """
    ...
    for i in range(n_bootstrap):
        ...
        boot_non_inter = np.random.choice(non_interaction_vi, size=len(non_interaction_vi), replace=True)
...
def plot_kurtosis_results(bootstrap_results, save_path=None, dpi=300):
    ...
    sns.histplot(distributions['interacting'], kde=True, color=colors['interacting'], 
                 alpha=0.6, label='Interacting', ax=ax1)
    sns.histplot(distributions['non_interacting'], kde=True, color=colors['non_interacting'], 
                 alpha=0.6, label='Non-interacting', ax=ax1)
    ...
    sns.histplot(distributions['differences'], kde=True, color=colors['difference'], 
                 alpha=0.7, ax=ax2)
    ax2.axvline(observed['difference'], color='black', linestyle='-', 
                label=f'Observed Diff: {observed["difference"]:.3f}')
    ax2.axvline(boot_results['confidence_interval'][0], color='red', linestyle='--', 
...
def plot_bootstrap_distribution(bootstrap_results):
    ...
    ci_lower = bootstrap_results.get('bootstrap_statistics', {}).get('confidence_interval_95', (4.32, 7.50))[0]
    ci_upper = bootstrap_results.get('bootstrap_statistics', {}).get('confidence_interval_95', (4.32, 7.50))[1]
    ...
    ax.axvline(x=observed_diff, color='red', linestyle='-', linewidth=2, 
               label=f'Observed Difference: {observed_diff:.2f}')
    
    # Add vertical lines for confidence interval
    ax.axvline(x=ci_lower, color='red', linestyle='--', linewidth=1.5, 
    ...
    if isinstance(p_value, (int, float)) and p_value < 0.001:
        p_value_text = f'p < {1/len(bootstrap_diffs):.1e}' if p_value == 0 else f'p = {p_value:.1e}'
    else:
    ...
    ax.set_title('Bootstrap Distribution of Kurtosis Difference\n(Interacting - Non-interacting Loci)', fontsize=14)
...
def plot_qq_with_bands(vi_matrix, interaction_loci_indices, non_interaction_loci_indices):
    ...
    for i in range(n_bootstrap):
        ...
        if n_inter <= n_non_inter:
            ...
            boot_non_inter_quantiles = np.linspace(min(boot_inter_quantiles), max(boot_inter_quantiles), n_non_inter)
            bootstrap_lines[i] = np.interp(boot_non_inter_quantiles, boot_inter_quantiles, boot_inter)
        else:
            boot_non_inter_quantiles = np.arange(1, n_non_inter + 1) / (n_non_inter + 1)
            boot_inter_quantiles = np.linspace(min(boot_non_inter_quantiles), max(boot_non_inter_quantiles), n_inter)
    ...
    ax.fill_between(x_values, lower_band, upper_band, alpha=0.2, color='#B9AFA7', label='95% Confidence Band')
    
    # Highlight points outside the bands
    outside_mask = (y_values < lower_band) | (y_values > upper_band)
    if np.any(outside_mask):
        ax.scatter(x_values[outside_mask], y_values[outside_mask], color='F8C5C1', s=50, alpha=0.7, 
                   label='Outside 95% CI')
    ...
    ax.text(0.05, 0.95, f"Interacting Kurtosis: {kurt_inter:.2f}\nNon-interacting Kurtosis: {kurt_non_inter:.2f}",
...
def known_interaction_indices(i_matrix):
   ...
   for phenotype in i_matrix:
      for interaction in phenotype:
         transformed_coordinates = ((interaction[0]*3)+interaction[2],(interaction[1]*3)+interaction[2])
