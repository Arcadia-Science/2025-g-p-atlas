...
def calculate_optimized_hybrid_vida_scores(vi_matrix, all_loci_indices, 
                                         z_threshold=2.0, 
    ...
    for i in range(n_samples):
        for locus in locus_partners:
            ...
            partner_clumpiness = calculate_clumpiness(partner_z, clumpiness_method, clumpiness_percentile)
            non_partner_clumpiness = calculate_clumpiness(non_partner_z, clumpiness_method, clumpiness_percentile)
    ...
    for locus in all_loci_indices:
        ...
        locus_samples = [m for m in sample_metrics if m['locus'] == locus and m['sample_idx'] in extreme_samples]
...
def optimize_vida_parameters(vi_matrix, all_loci_indices, 
                           allele_interaction_indices, allele_direct_indices,
    ...
    print(f"Testing {len(z_thresholds)} z-thresholds × {len(clumpiness_percentiles)} percentiles × {len(clumpiness_methods)} methods")
    print(f"Total combinations: {len(z_thresholds) * len(clumpiness_percentiles) * len(clumpiness_methods)}")
    print("-" * 60)
    
    results = []
    
    # Test all parameter combinations
    for z_thresh, clump_pct, clump_method in product(z_thresholds, clumpiness_percentiles, clumpiness_methods):
        try:
            print(f"Testing: z_threshold={z_thresh}, clumpiness_pct={clump_pct}, method={clump_method}")
            
            # Calculate metrics with current parameters
            metrics_df = calculate_optimized_hybrid_vida_scores(
                vi_matrix, all_loci_indices,
            ...
            known_loci_mask = labeled_df['locus'].isin(np.concatenate([allele_interaction_indices, allele_direct_indices]))
    ...
    print(f"Parameters: z_threshold={overall_best['z_threshold']}, "
    ...
    for metric in clump_perf['metric'].unique():
        ...
        axes[0, 1].plot(metric_data['clumpiness_percentile'], metric_data['auc'], 'o-', label=metric)
    ...
    for i, metric in enumerate(metrics):
        ...
        aucs = [metric_data[metric_data['clumpiness_method'] == m]['auc'].iloc[0] 
                if len(metric_data[metric_data['clumpiness_method'] == m]) > 0 else 0 
                for m in methods]
...
def plot_vida_performance(metrics_df, allele_interaction_indices, allele_direct_indices, 
                         metric_name='vida_score', title_suffix=''):
    """
    ...
    ax1.plot([0, 1], [0, 1], lw=2, linestyle='--', alpha=0.6,
    ...
    ax1.text(0.6, 0.2, f'Sensitivity: {sensitivity:.3f}\nSpecificity: {specificity:.3f}', 
    ...
    enrichment_values = []
    interacting_loci_values = [sorted_df[metric_name][n] for n in range(n_total) if sorted_df['is_interaction'][n]==1]
    ...
    ax2.axhline(y=expected_pct, linestyle='--',  
                label=f'Expected (random) = {expected_pct:.1f}%')
    ...
    ax2.text(60, 80, f'Top 10%: {top_10_pct:.1f}% ({enrichment_10:.1f}x)\nTop 5%: {top_5_pct:.1f}% ({enrichment_5:.1f}x)', 
             bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.8),
             )
    ...
    vida_int = [vida_metrics['vida_score'][n] for n in range(len(vida_metrics['vida_score'])) if vida_metrics['locus'][n] in allele_interaction_indices]
    vida_non_int = [vida_metrics['vida_score'][n] for n in range(len(vida_metrics['vida_score'])) if vida_metrics['locus'][n] in allele_direct_only_indices]
...
def hypergeometric_enrichment_test(vida_results, allele_interaction_indices, allele_direct_indices, 
                                 metric_name='VIDA_score', top_percentiles=[5, 10, 20]):
    """
    ...
    for percentile in top_percentiles:
        ...
        print(f"  Enrichment factor: {enrichment:.2f}x (95% CI: {enrichment_ci_lower:.2f}-{enrichment_ci_upper:.2f})")
    ...
    ax1.errorbar(enrichment_df['percentile'], enrichment_df['enrichment_factor'],
                yerr=[enrichment_df['enrichment_factor'] - enrichment_df['enrichment_ci_lower'],
    ...
    if len(sig_points) > 0:
        ax1.scatter(sig_points['percentile'], sig_points['enrichment_factor'], 
                   color='red', s=100, alpha=0.7, zorder=5)
    
    # Plot 2: P-values
    ax2.bar(enrichment_df['percentile'], -np.log10(enrichment_df['p_value']), 
    ...
    if len(significant_tests) > 0:
        ...
        print(f"  Top {best_enrichment['percentile']}%: {best_enrichment['enrichment_factor']:.2f}x enrichment")
        print(f"  P-value: {best_enrichment['p_value']:.2e}")
        print(f"  95% CI: {best_enrichment['enrichment_ci_lower']:.2f}-{best_enrichment['enrichment_ci_upper']:.2f}")
    else:
...
def example_enrichment_test():
    ...
    print("clumpiness_enrichment = hypergeometric_enrichment_test(..., metric_name='avg_clumpiness_diff')")
    print("extreme_ratio_enrichment = hypergeometric_enrichment_test(..., metric_name='avg_extreme_ratio_diff')")

# Advanced analysis function
def multiple_metric_enrichment_test(vida_results, allele_interaction_indices, allele_direct_indices,
    ...
    for metric in metrics:
        if metric in vida_results.columns:
            ...
            results = hypergeometric_enrichment_test(
                vida_results, allele_interaction_indices, allele_direct_indices,
                metric_name=metric, top_percentiles=[5, 10, 20]
            )
    ...
    for metric, results in all_results.items():
        if results is not None:
            ...
            print(f"{metric}: {best['enrichment_factor']:.2f}x at top {best['percentile']}% (p={best['p_value']:.2e})")
