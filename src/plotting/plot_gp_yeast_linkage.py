...
yeast_linkage_data = open(target_folder+'../41586_2013_BFnature11867_MOESM88_ESM_yeat_linkage_data.csv','r').read().split('\n')
...
#cluster yeast loci influencing phenotypes. Join genetic regions based on overlapping confidence intervals
locus_cluster_ids = [1]
current_id = 1
...
for n in range(len(sorted_linkage_data)-1):
 if int(sorted_linkage_data[n+1][5]) < int(sorted_linkage_data[n][6]) and int(sorted_linkage_data[n+1][2])==current_chromosome:
   locus_cluster_ids.append(current_id)
   chromosome_labels.append(current_chromosome)
 elif int(sorted_linkage_data[n+1][5]) >= int(sorted_linkage_data[n][6]) and int(sorted_linkage_data[n+1][2])==current_chromosome:
   current_id += 1
   locus_cluster_ids.append(current_id)
...
clusters, chr_labels_by_cluster = zip(*sorted(list(set(zip(locus_cluster_ids,chromosome_labels))),key = lambda x: x[0]))

#start plotting the genetic linkage figure
plt.figure(figsize=(15, 5.5))

#plot positions of linked intervals
absolute_interval_start = [marker_absolute_position_dict[str(chr_labels_by_cluster[n])+'_'+str(interval_start[n])] for n in range(len(interval_start))]
absolute_interval_stop = [marker_absolute_position_dict[str(chr_labels_by_cluster[n])+'_'+str(interval_stop[n])] for n in range(len(interval_stop))]
...
#format the plot
plt.plot([sorted_absolute_marker_position[n] for n in sorted_linked_marker_index],sorted_variance_explained,'o', markersize = 2)
...
for n in range(0,len(chromosome_start)-1,2):
 plt.plot([chromosome_start[n],chromosome_start[n+1]],[0,0],'k',linewidth=8,solid_capstyle='butt')

for n in range(1,len(chromosome_start),2):
 plt.plot([chromosome_start[n],chromosome_start[n+1]],[0,0],'r',linewidth=8,solid_capstyle='butt')
plt.axhline(percentile,linestyle='dotted',c='k')
plt.xticks([chromosome_start[n]+(chromosome_start[n+1]-chromosome_start[n])/2 for n in range(len(chromosome_start)-1)],[str(x) for x in range(1,17)])
...
max_attr_per_cluster = [max([max_attrs[n] for n in range(len(max_attrs)) if sorted_absolute_marker_position[n] >absolute_interval_start[m] \
and sorted_absolute_marker_position[n] <absolute_interval_stop[m]]) for m in range(len(absolute_interval_start))]

max_attr_per_non_linked = [[max_attrs[n] for n in range(len(max_attrs)) if sorted_absolute_marker_position[n] < absolute_interval_start[m] \
and sorted_absolute_marker_position[n] > absolute_interval_stop[m-1]] for m in range(1,len(absolute_interval_start))]
max_attr_per_non_linked = [[]]+max_attr_per_non_linked+[[max_attrs[n] for n in range(len(max_attrs)) if sorted_absolute_marker_position[n]\
 > absolute_interval_stop[-1]]]

max_attr_per_non_linked = [max(x) if len(x)>0 else 0 for x in max_attr_per_non_linked]

scores = [item for sublist in zip(max_attr_per_non_linked,max_attr_per_cluster+[[]]) for item in sublist][:-1]
...
abs_marker_conf_start_stop = [[marker_absolute_position_dict[x[2]+'_'+x[5]],marker_absolute_position_dict[x[2]+'_'+x[6]]]\
 for x in sorted_linkage_data]
attr_per_locus = [[max_attrs[n] for n in range(len(max_attrs)) if sorted_absolute_marker_position[n] >= m[0] \
and sorted_absolute_marker_position[n] <= m[1]] for m in abs_marker_conf_start_stop]
...
sns.jointplot(x=np.array(sorted_variance_explained)/max(sorted_variance_explained),y=np.array(max_attr_per_locus)/max(max_attr_per_locus),hue=0)
print(sc.stats.pearsonr(np.array(sorted_variance_explained)/max(sorted_variance_explained),np.array(max_attr_per_locus)/max(max_attr_per_locus)))
plt.plot([0,1],[0,1],':k')
plt.xlim(0,1.01)
plt.ylim(0,1.01)
...
cumulative_fraction = [len([x for x in max_attr_per_locus if x<y])/len(max_attr_per_locus) for y in percentiles]
...
sns.jointplot(x=np.array(sorted_variance_explained)/max(sorted_variance_explained),y=np.array(max_attr_per_linked_locus)/max(max_attr_per_linked_locus),hue=0)
print(sc.stats.pearsonr(np.array(sorted_variance_explained)/max(sorted_variance_explained),np.array(max_attr_per_linked_locus)/max(max_attr_per_linked_locus)))
