...
args.add_argument(
    ...
    type=int,
    default=32,
    help="number of neurons in the hidden layers of gencoder to decoder network",
)
args.add_argument(
    "--batchnorm_momentum",
    type=float,
...
args.add_argument(
    "--g_latent_dim", type=int, default=32, help="number of neurons in the genetic latent space"
)
args.add_argument(
...
args.add_argument(
    ...
    type=int,
    default=100,
    help="number of epochs to train the genetic autoencoder",
)
args.add_argument(
    "--n_epochs_gen_phen",
    type=int,
    default=100,
    help="number of epochs to train the genotype-phenotype map",
)
args.add_argument(
    "--n_loci_measured",
    type=int,
...
args.add_argument(
    ...
    type=str,
    default=None,
    help="path to the genotype decoder weights to use at initialization",
)
args.add_argument(
    "--hot_start_path_ged",
    type=str,
    default=None,
    help="path to the genotype to phenotype interstitial weights to use at initialization",
)
...
train_data_geno = dataset_geno(dataset_path + train_dat, n_geno=vabs.n_loci_measured)
test_data_geno = dataset_geno(dataset_path + test_dat, n_geno=vabs.n_loci_measured)

train_data_phen_geno = dataset_phen_geno(
    dataset_path + train_dat, n_geno=vabs.n_loci_measured, n_phens=vabs.n_phens_to_analyze
)
test_data_phen_geno = dataset_phen_geno(
...
class GQ_net(nn.Module):
    def __init__(self, n_loci=None, N=None):
        ...
        batchnorm_momentum = vabs.batchnorm_momentum
        g_latent_dim = vabs.g_latent_dim
        self.encoder = nn.Sequential(
...
if vabs.hot_start is True:
    ...
    GQ.load_state_dict(torch.load(vabs.hot_start_path_gd, weights_only=True), strict=False)
    GQP.load_state_dict(torch.load(vabs.hot_start_path_ged, weights_only=True), strict=False)
...
fa = FeatureAblation(sequential_forward_attr_gen_phen)
...
for dat in test_loader_phen_geno:
    ...
    # print(fa.attribute(inputs=(gt,ph)))
